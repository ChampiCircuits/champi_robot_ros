...
const osThreadAttr_t myTaskModbusA_attributes = {
    ...
    .stack_size = 128 * 8 // TODO 4 originally
};
...
// This function must be called only after disabling USART RX interrupt or
// inside of the RX interrupt
...
void ModbusInit(modbusHandler_t *modH) {
  ...
  if (numberHandlers < MAX_M_HANDLERS) {
    ...
    if (modH->uModbusType == MB_SLAVE) {
      ...
      modH->xTimerTimeout = xTimerCreate(
          "xTimerTimeout",  // Just a text name, not used by the kernel.
          modH->u16timeOut, // The timer period in ticks.
          pdFALSE, // The timers will auto-reload themselves when they expire.
          (void *)modH->xTimerTimeout, // Assign each timer a unique id equal to
                                       // its array index.
          (TimerCallbackFunction_t)
              vTimerCallbackTimeout // Each timer calls the same callback when
                                    // it expires.
      );
    ...
    modH->xTimerT35 = xTimerCreate(
        "TimerT35", // Just a text name, not used by the kernel.
        T35,        // The timer period in ticks.
        pdFALSE,    // The timers will auto-reload themselves when they expire.
        (void *)modH->xTimerT35, // Assign each timer a unique id equal to its
                                 // array index.
        (TimerCallbackFunction_t)vTimerCallbackT35 // Each timer calls the same
...
void ModbusStart(modbusHandler_t *modH) {
  ...
  if (modH->xTypeHW == USART_HW_DMA && ENABLE_USART_DMA == 0) {
    while (1)
      ; // ERROR To use USART_HW_DMA you need to enable it in the ModbusConfig.h
        // file
  }
  ...
  if (modH->xTypeHW == USART_HW || modH->xTypeHW == USART_HW_DMA) {
    ...
    if (modH->xTypeHW == USART_HW_DMA) {
      ...
      if (HAL_UARTEx_ReceiveToIdle_DMA(modH->port, modH->xBufferRX.uxBuffer,
                                       MAX_BUFFER) != HAL_OK) {
        while (1) {
          // error in your initialization code
        }
      }
      __HAL_DMA_DISABLE_IT(modH->port->hdmarx,
...
bool TCPwaitConnData(modbusHandler_t *modH) {
  ...
  // NULL means there is a free connection slot, so we can accept an incoming
  // client connection
  ...
  if (modH->xTypeHW == TCP_HW) {
    ...
    if (modH->conn != NULL) {
      ...
      if (err == ERR_OK) {
        ...
        netconn_set_recvtimeout(modH->conn,
                                1); // this is necessary to make it non blocking
      } else {
        while (1) {
...
void StartTaskModbusSlave(void *argument) {
  ...
  for (;;) {
    ...
    if (modH->xTypeHW == USB_CDC_HW) {
      ulTaskNotifyTake(
          pdTRUE,
          portMAX_DELAY); /* Block indefinitely until a Modbus Frame arrives */
    ...
    if (modH->xTypeHW == TCP_HW) {
      ...
      if (TCPwaitConnData(modH) ==
          false) // wait for connection and receive data
      {
...
#else
      if (modH->xTypeHW != TCP_HW) {
        continue; // for Modbus TCP this is not validated, user should modify
    ...
    modH->i8lastError = 0;
    xSemaphoreTake(
        modH->ModBusSphrHandle,
        portMAX_DELAY); // before processing the message get the semaphore
 ...
 * The Master must be in COM_IDLE mode. After it, its state would be
 * COM_WAITING. This method has to be called only in loop() section.
 *
 * @see modbus_t
 * @param modH  modbus handler
    ...
    if (err == ERR_OK) {
      ...
      if (err == ERR_OK) {
        if ((buflen > 11 && (modH->uModbusType == MB_SLAVE)) ||
            (buflen >= 10 && (modH->uModbusType ==
                              MB_MASTER))) // minimum frame size for modbus TCP
        {
          if (buf[2] == 0 || buf[3] == 0) // validate protocol ID
          {
            uLength = (buf[4] << 8 & 0xff00) | buf[5];
  ...
  for (;;) {
    ...
    if (modH->xTypeHW == TCP_HW) {
      ...
      if (ulNotificationValue == ERR_OK) {
        ...
        ulNotificationValue =
            TCPgetRxBuffer(modH); // TCP receives the data and the notification
                                  // simultaneously since it is synchronous
    ...
    if (modH->xTypeHW ==
        USART_HW) // TCP and USB_CDC use different methods to get the buffer
    {
      getRxBuffer(modH);
    }
    ...
    if (modH->i8lastError == 0) // no error the error_OK, we need to use a
                                // different value than 0 to detect the timeout
    {
      xSemaphoreGive(modH->ModBusSphrHandle); // Release the semaphore
...
int16_t getRxBuffer(modbusHandler_t *modH) {
  ...
  if (modH->xTypeHW == USART_HW) {
    HAL_UART_AbortReceive_IT(modH->port); // disable interrupts to avoid race
                                          // conditions on serial port
  }
...
uint8_t validateRequest(modbusHandler_t *modH) {
  ...
  case MB_FC_WRITE_MULTIPLE_COILS:
    ...
    if (word(modH->u8Buffer[NB_HI], modH->u8Buffer[NB_LO]) % 8)
      u16NRegs++;
    u16NRegs =
        u16NRegs + 5; // adding the header  and CRC ( Slave address + Function
                      // code  + number of data bytes to follow + 2-byte CRC )
    if (u16NRegs > 256)
 ...
 */
static void sendTxBuffer(modbusHandler_t *modH) {
  // when in slaveType and u8AddressMode == ADDRESS_BROADCAST, do not send
  // anything
  if (modH->uModbusType == MB_SLAVE &&
      modH->u8AddressMode == ADDRESS_BROADCAST) {
    modH->u8BufferSize = 0;
  ...
  if (modH->xTypeHW == USART_HW || modH->xTypeHW == USART_HW_DMA) {
    ...
    if (modH->EN_Port != NULL) {
      // enable transmitter, disable receiver to avoid echo on RS485
      // transceivers
 ...
 */
    ...
    defined(STM32L082xx) || defined(STM32F7) || defined(STM32WB) ||            \
    defined(STM32G070xx) || defined(STM32F0) || defined(STM32G431xx) ||        \
    defined(STM32H5)
  ...
  while ((modH->port->Instance->SR & USART_SR_TC) == 0)
#endif
    {
      // block the task until the the last byte is send out of the shifting
      // buffer in USART
    }
    ...
    CDC_Transmit_HS(modH->u8Buffer, modH->u8BufferSize);
    // set timeout for master query
    if (modH->uModbusType == MB_MASTER) {
      xTimerReset(modH->xTimerTimeout, 0);
    }
    ...
    u8MBAPheader[0] =
        highByte(modH->u16TransactionID); // this might need improvement the
                                          // transaction ID could be validated
...
int8_t process_FC16(modbusHandler_t *modH) {
  ...
  modH->u8Buffer[NB_LO] =
      (uint8_t)u16regsno; // answer is always 256 or less bytes
  modH->u8BufferSize = RESPONSE_SIZE;
