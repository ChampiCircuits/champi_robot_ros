...
PosStepper::PosStepper(const TIM_HandleTypeDef &tim_handle_step,
                       ...
                       const uint16_t gpio_pin_dir) :
_speed_stepper(
    tim_handle_step, tim_channel_step,
    gpio_port_dir, gpio_pin_dir) {
...
void PosStepper::set_max_speed(float speed) {
  _max_speed = speed;
}

void PosStepper::set_max_accel(float accel) {
  _max_accel = accel;
}
...
bool PosStepper::pos_reached() const {
  return _pos_reached;
}
...
void PosStepper::spin_once(double dt) {
  ...
  const float limited_speed = limit_accel(
    _speed_stepper.get_speed_rps(),
    _max_speed * (delta_pos > 0 ? 1. : -1.),
    _max_accel, dt);
