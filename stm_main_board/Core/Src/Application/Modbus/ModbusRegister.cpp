...
void setup_registers() {
  ...
  measured_vel =
      (Vector3 *)init_ptr_to_register(sizeof(Vector3) / sizeof(uint16_t));
  otos_pose =
      (Vector3 *)init_ptr_to_register(sizeof(Vector3) / sizeof(uint16_t));
  stm_config =
      (StmConfig *)init_ptr_to_register(sizeof(StmConfig) / sizeof(uint16_t));

#ifdef MODBUS_MASTER
  init_register_metadata(reg_cmd_vel, (uint16_t *)cmd_vel,
                         sizeof(Vector3) / sizeof(uint16_t));
  init_register_metadata(reg_measured_vel, (uint16_t *)measured_vel,
                         sizeof(Vector3) / sizeof(uint16_t));
  init_register_metadata(reg_otos_pose, (uint16_t *)otos_pose,
                         sizeof(Vector3) / sizeof(uint16_t));
  init_register_metadata(reg_stm_config, (uint16_t *)stm_config,
                         sizeof(StmConfig) / sizeof(uint16_t));
#endif // MODBUS_MASTER
