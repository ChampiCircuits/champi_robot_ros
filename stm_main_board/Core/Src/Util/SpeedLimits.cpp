...
double limit_accel_decel(double current_speed, double goal_speed,
  ...
  } else if ((goal_speed > current_speed && current_speed >= 0) ||
             (goal_speed < current_speed && current_speed <= 0)) {
    // We are accelerating
...
double limit_accel(double current_speed, double goal_speed,
...
double limit_decel(double current_speed, double goal_speed,
                   double max_deceleration, double dt) {
  // check if goal is smaller or greater than current speed
  if (goal_speed > current_speed) {
    // Compute the speed we can reach in dt
