...
struct __attribute__((packed)) Actuators
{
  ...
  uint8_t left_suction_cups_activation_for_request[static_cast<size_t>(ActuatorCommand::ACTUATORS_COUNT)]; 
  // 4 bools, 1 for each suction cup. numbers are cups from right to left (from the point of view of the robot)
  uint8_t right_suction_cups_activation_for_request[static_cast<size_t>(ActuatorCommand::ACTUATORS_COUNT)]; 
};
