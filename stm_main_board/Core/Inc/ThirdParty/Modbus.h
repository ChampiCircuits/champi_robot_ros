 ...
 * This structure contains all the necessary fields to make the Master generate
 * a Modbus query. A Master may keep several of these structures and send them
 * cyclically or use them according to program needs.
 */
typedef struct {
  ...
  uint32_t *u32CurrentTask; /*!< Pointer to the task that will receive
...
typedef struct {
  ...
  GPIO_TypeDef
      *EN_Port;    //!< flow control pin: 0=USB or RS-232 mode, >1=RS-485 mode
...
void ModbusQuery(modbusHandler_t *modH,
                 modbus_t telegram); // put a query in the queue tail
void ModbusQueryInject(modbusHandler_t *modH,
                       modbus_t telegram);  // put a query in the queue head
...
void ModbusCloseConnNull(modbusHandler_t *modH); // close the TCP connection and
                                                 // cleans the modbus handler
#endif

// Function prototypes for ModbusRingBuffer
void RingAdd(modbusRingBuffer_t *xRingBuffer,
             uint8_t u8Val); // adds a byte to the ring buffer
uint8_t
RingGetAllBytes(modbusRingBuffer_t *xRingBuffer,
                uint8_t *buffer); // gets all the available bytes into buffer
                                  // and return the number of bytes read
uint8_t
RingGetNBytes(modbusRingBuffer_t *xRingBuffer, uint8_t *buffer,
              uint8_t uNumber); // gets uNumber of bytes from ring buffer,
                                // returns the actual number of bytes read
uint8_t RingCountBytes(
    modbusRingBuffer_t *xRingBuffer); // return the number of available bytes
...
extern uint8_t numberHandlers; // global variable to maintain the number of
                               // concurrent handlers
...
void ModbusEnd(); //!<finish any communication and release serial communication
port
