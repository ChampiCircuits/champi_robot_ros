Les PAMIs (petit actionneur mobile indépendant) dans la coupe de france de robotique sont des petits robots autonomes qui participent à la coupe de france de robotique. 



Dans ce dossier, on va créer le code pour faire fonctionner les PAMIs avec une carte ESP32.
Mais d'abord, on veut faire du tooling
En fait les PAMIs auront tous le même code, mais des trajectoires différentes.

Donc on va faire une interface web de configuration pour les PAMIs, qui permettra de configurer les trajectoires et les paramètres de chaque PAMI.
Elle permettra aussi de flasher le code sur les PAMIs
Elle permettra de définir des trajectoires en cliquant sur la carte (image de la table dispo dans le dossier (3m en x par 2m en y)) et en définissant des points de passage.
Ces trajectoires pourront ensuite être sauvegardées et compilées dans le code des PAMIs avant de les flasher.
On voudra pouvoir visualiser les trajectoires sur la carte pour vérifier qu'elles sont correctes. Soit trajectoire par trajectoire, soit toutes les trajectoires en même temps pour voir les interactions entre les PAMIs.
Et évidemment pouvoir jouer les trajectoires en visualisation en temps réel pour vérifier que les PAMIs ne se rentrent pas dedans et que les trajectoires sont réalisables.
