"""
Planner — ROS-free path-planning state machine.

The ``Planner`` class contains all navigation loop logic that was previously
embedded in ``PlannerNode.execute_callback``.  It is driven by the node calling
``step(robot_pose)`` once per loop iteration; it returns a :class:`StepOutput`
that the node dispatches (publish ctrl goal, publish feedback, handle terminal
action result, etc.).

ROS2 is *not* imported here.  ``Navigate.Goal`` is accepted as-is because it is
a plain Python dataclass generated by rosidl — it carries no ROS runtime.
"""
...
# ---------------------------------------------------------------------------
# Planning diagnostics (moved from PlannerNode)
# ---------------------------------------------------------------------------
...
class PlanningDiagnostic:
    ...
    def produce_diagnostics(self, stat):
        ...
        stat.summary(
            level,
            f"last={self._last_ms:.1f}ms  avg={avg_ms:.1f}ms  worst={self._worst_ms:.1f}ms",
        )
...
class Planner:
    ...
    # ------------------------------------------------------------------
    # Read-only accessors (for diagnostics / viz when idle)
    # ------------------------------------------------------------------
    ...
    def get_all_obstacles(self):
        """Convenience pass-through for the node's visualisation callback."""
        return self._obstacle_manager.get_all_obstacles()

    def build_expanded_obstacles(self, obstacles):
        """Convenience pass-through for the node's visualisation callback."""
        return self._visibility_planner.build_expanded_obstacles(obstacles)
